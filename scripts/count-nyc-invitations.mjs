#!/usr/bin/env node
/**
 * Count NYC wedding invitation households.
 *
 * Logic:
 * 1. Fetch all guests with 'NYC' in their Event Invitations
 ...
 *
 * Usage: node scripts/count-nyc-invitations.mjs
...
async function main() {
  ...
  const nycPages = allPages.filter(isNYC);
  console.log(`NYC-invited guests: ${nycPages.length}`);
  ...
  // Keep only components that contain at least one NYC guest
  // For display, show only the NYC members per household
  const households = Object.values(components)
    .filter(members => members.some(isNYC))
    .map(members => ({
      nycMembers: members.filter(isNYC),
      allMembers: members,
    }));

  // Sort by NYC member count descending
  households.sort((a, b) => b.nycMembers.length - a.nycMembers.length);

  // --- Report ---
  console.log(`\n${'='.repeat(60)}`);
  console.log(`NYC INVITATION COUNT REPORT`);
  console.log('='.repeat(60));
  console.log(`\nNYC guests:         ${nycPages.length}`);
  console.log(`Total households:   ${households.length}  ← invitations to print`);

  // Size distribution (by NYC member count)
  const sizeMap = {};
  for (const h of households) {
    const n = h.nycMembers.length;
    sizeMap[n] = (sizeMap[n] || 0) + 1;
  }
  console.log('\nHousehold size breakdown (NYC members per invitation):');
  for (const [size, count] of Object.entries(sizeMap).sort((a, b) => Number(a[0]) - Number(b[0]))) {
    const label = size === '1' ? 'solo' : `${size} NYC guests`;
  ...
  for (const h of multi) {
    ...
    const franceOnly = h.allMembers.filter(m => !isNYC(m));
  ...
  // Outliers: households of 4+ NYC members
  const large = households.filter(h => h.nycMembers.length >= 4);
  if (large.length > 0) {
    console.log(`\n⚠️  Large households (4+ NYC guests) — double-check these:`);
